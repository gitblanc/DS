...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        formulario.addCampo(new Campo("Código de Producto", text -> text.length() == 4));
        ...
        formulario.addCampo(new Campo("Edad", new CheckAll(new CheckNumber(), text -> Integer.parseInt(text) > 18)));

        // "Sueldo". Dígitos y mayor que 800 y menor que 1200
        formulario.addCampo(
                new Campo("Sueldo", new CheckAll(
                        new CheckNumber(), new CheckGreater(800), new CheckLess(1200))));

        // "Ubicación". "Santander\Oviedo\Cádiz" o Código Postal (ver arriba)
        formulario.addCampo(new Campo("Ubicación",
                new CheckAny(
                        new CheckValues("Santander", "Oviedo", "Cádiz"),
                        new CheckCodigoPostal())));

        // "Código de Promoción". Texto (no dígitos) o (número de 3 digitos)
